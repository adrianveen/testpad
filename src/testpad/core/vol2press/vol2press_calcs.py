...
class Vol2Press:
    def __init__(self, cal_eb50_file, sys_eb50_file, sweep_txt, freq) -> None:
        ...
        self.closest_cal_freq, self.cal_eb50_dict = self.closest_frequency(
            self.freq, cal_eb50_file
        )  # find closest freq dict in file
        self.closest_sys_freq, self.sys_eb50_dict = self.closest_frequency(
            self.freq, sys_eb50_file
        )

        self.sweep_array = np.loadtxt(
            sweep_txt, delimiter=",", skiprows=5
        )  # skip first five rows of file because they're headers
        self.input_mV = self.sweep_array[:, 3]
        self.neg_pressure = self.sweep_array[:, 0]
        self.cal_avg_gain = np.mean(
            self.cal_eb50_dict["gain"]
        )  # get average gain from gain list in closest frequency
