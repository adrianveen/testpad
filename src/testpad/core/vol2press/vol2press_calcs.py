...
class Vol2Press():
    def __init__(self, cal_eb50_file, sys_eb50_file, sweep_txt, freq) -> None:
        ...
        self.sweep_array = np.loadtxt(sweep_txt, delimiter=",", skiprows=5) # skip first five rows of file because they're headers 
        self.input_mV = self.sweep_array[:, 3] 
        self.neg_pressure = self.sweep_array[:, 0] 
        self.cal_avg_gain = np.mean(self.cal_eb50_dict['gain']) # get average gain from gain list in closest frequency 
