...
from testpad.ui.tabs.degasser_tab.widgets.delegates import (
    MeasuredValueDelegate,
    TimeSeriesValueDelegate,
)
from testpad.ui.tabs.degasser_tab.widgets.table_widgets import (
    ColumnMajorTableWidget,
)
...
class DegasserTab(BaseTab):
    ...
    def show_file_open_dialog(self, title: str, filter_str: str) -> str | None:
        """Show file open dialog and return the selected path.

        Args:
            title: Dialog title.
            filter_str: File filter string (e.g., "CSV Files (*.csv)").
    ...
    def show_file_save_dialog(
        self, title: str, default_name: str, filter_str: str
    ) -> str | None:
    ...
    def _build_test_table(self) -> QWidget:
        ...
        self._test_table.setItemDelegateForColumn(
            MEASURED_COL_INDEX,
            MeasuredValueDelegate(units_by_row, specs_by_row, self._test_table),
        )
    ...
    def _build_time_series_table(self) -> QWidget:
        ...
        self._time_series_widget.setItemDelegateForRow(
            1, TimeSeriesValueDelegate(self._time_series_widget)
        )
