...
from testpad.utils.add_ncycle_sweep_data_to_config_file import (
    add_ncycle_sweep_to_transducer_file,
)
...
class Vol2PressTab(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        """Initialize the "Sweep Analysis" tab."""
        ...
        cal_eb50_file_btn.clicked.connect(lambda: self._open_file_dialog("eb-50_cal"))
        sys_eb50_file_btn = QPushButton("SELECT CUSTOMER EB-50 YAML")
        sys_eb50_file_btn.clicked.connect(lambda: self._open_file_dialog("eb-50_sys"))
        save_btn = QPushButton("SAVE AS")
        save_btn.clicked.connect(lambda: self._open_file_dialog("save"))
        ...
        self.one_time_fields_list = [
            self.transducer_field,
            self.impedance_field,
            self.phase_field,
            self.pcd_field,
            self.diameter_field,
        ...
        sweep_btn.clicked.connect(lambda: self._open_file_dialog("sweep"))
        ...
        self.acoustic_efficiency_field.setToolTip(
            "Acoustic efficiency in percentage, from 0.00 to 100"
        )
        ...
        self.fields_list = [
            ...
            self.offset_field_3,
            self.uni_cali_field,
            self.acoustic_efficiency_field,
        ]
        ...
        self.browse_ncycles_data.setSizePolicy(
            QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Fixed
        )
        self.browse_ncycles_data.clicked.connect(
            lambda: self._open_file_dialog("n_cycles")
        )
        ...
        self.results_btn.clicked.connect(lambda: self._create_yaml())

        self.save_svg_btn = QPushButton("SAVE PNP AS SVG")
        self.save_svg_btn.clicked.connect(lambda: self._open_file_dialog("save_svg"))
        ...
        main_layout.addWidget(
            cycles_checkbox, 4, 0, 1, 1, alignment=Qt.AlignmentFlag.AlignCenter
        )
    ...
    def clear_dicts(self) -> None:
        ...
        if (
            qReply == QMessageBox.StandardButton.Yes
        ):  # if user says yes, clear previous dicts
            self.freq_dict = {}
            self.values_dict = {}
            self.summary_dict = {}
    ...
    def get_calcs(self) -> None:
        if (
            ...
            and self.sys_eb50_file is not None
        ):
            self.calcs = Vol2Press(
                self.cal_eb50_file,
                self.sys_eb50_file,
                self.sweep_file,
                float(self.freq_field.text()),
            )
            self.r_value = float(f"{self.calcs.return_B_value():.4f}")
            self.text_display.append(f"Regression value: {self.r_value} MPa/Vpp\n")
            self._print_graphs(dataset=self.calcs)
            self._add_to_frequency()
        else:
            if self.sweep_file is None:
                self.text_display.append("Missing file: sweep file")
            if self.cal_eb50_file is None:
                self.text_display.append("Missing file: calibration EB-50 file")
            if self.sys_eb50_file is None:
                self.text_display.append("Missing file: customer EB-50 file")
    ...
    @Slot()
    def _add_to_frequency(self) -> None:
        freq = float(self.freq_field.text()) * 1000000  # MHz to Hz
        self.text_display.append(f"Adding {freq} Hz information to dictionary...\n")
        spec_freq_dict = {}
        self._add_to_dict("vol2press", self.r_value, spec_freq_dict)
        self._add_to_dict(
            "TxAxialFocalDiameter", self.axial_field.text(), spec_freq_dict
        )
        self._add_to_dict(
            "TxLateralFocalDiameter", self.lateral_field.text(), spec_freq_dict
        )
        self._add_to_dict(
            "offset",
            [
                self.offset_field_1.text(),
                self.offset_field_2.text(),
                self.offset_field_3.text(),
            ],
            spec_freq_dict,
        )
        self._add_to_dict(
            "unified_vol2press", self.uni_cali_field.text(), spec_freq_dict
        )
        self._add_to_dict(
            "acoustic_efficiency_percent",
            self.acoustic_efficiency_field.text(),
            spec_freq_dict,
        )
    ...
    def _create_yaml(self) -> None:
        available_frequencies = np.array(
            [key for key in self.freq_dict if type(key) is float]
        ).tolist()  # gets a list of the existing frequencies in the field
        self.values_dict["txAvailableFrequencies"] = available_frequencies

        self._add_to_dict(
            "impedance_fund", self.impedance_field.text(), self.values_dict
        )
        self._add_to_dict("phase_fund", self.phase_field.text(), self.values_dict)
        self._add_to_dict("PCD_freq", self.pcd_field.text(), self.values_dict)
        self._add_to_dict("txDiameter", self.diameter_field.text(), self.values_dict)
        self._add_to_dict(
            "txFocalPointFactor", self.focal_point_field.text(), self.values_dict
        )

        self.values_dict = (
            self.values_dict | self.freq_dict
        )  # ensure that all frequency dictionary values are placed at the end
        self.summary_dict = {}
        ...
        with Path(self.save_file_path).open("w", encoding="utf8") as f:
            ...
            self.text_display.append(
                "Writing to dictionary complete. YAML file created."
            )

        if not hasattr(self, "n_cycles_dir"):
            return
        self.n_cycles_plot_data = add_ncycle_sweep_to_transducer_file(
            self.n_cycles_dir, self.save_file_path
        )
        self._plot_ncycle_data(self.n_cycles_plot_data)

    def _plot_ncycle_data(self, plot_data: list | None = None) -> None:
        if plot_data is None:
            plot_data = []
        self.fig, self.ax = plt.subplots(1, 1)
        self.pnp_plot = FigureCanvas(self.fig)
        fus_colors = self._generate_color_palette("#74BEA3", len(plot_data))

        for i, (freq, cycles, pressure) in enumerate(plot_data):
            self.ax.plot(
                cycles, pressure, label=f"{freq / 1e6} MHz", color=fus_colors[i]
            )
        ...
        self.ax.set_title("Number of Cycles vs. Normalized Peak Negative Pressure")
