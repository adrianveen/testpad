...
class SweepGraphTab(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        """Initialize the Sweep Plot Tab View."""
        ...
        self.select_file_btn.clicked.connect(lambda: self._open_file_dialog("hdf5"))
        # print graph button
    ...
    def _open_file_dialog(self, d_type: str) -> None:
        ...
        # file saving location and save graph as SVG
            ...
            if self.dialog.exec():
                ...
                self.text_display.append(
                    "The following files were saved as SVG files:\n"
                )
                self.text_display.append(
                    f"Time Domain Graph: {self.scan_data_object.serial_no}\
                        _time_graph_{timestamp}.svg\n"
                )
                self.text_display.append(
                    f"FFT Graph: {self.scan_data_object.serial_no}\
                        _fft_graph_{timestamp}.svg\n"
                )
        ...
        if self.scan_data_hdf5 and len(self.scan_data_hdf5) > 0:
            ...
            with h5py.File(self.scan_data_hdf5[0], "r") as f:
                ...
                self.trace_no_menu.addItems(
                    [str(i + 1) for i in range(raw_pressu_waveforms.shape[0])]
                )
                # set the max index
    ...
    def _create_graph(self) -> None:
        if self.scan_data_hdf5 is not None:
            ...
            self.time_graph, self.fft_graph = self.scan_data_object.get_graphs(
                self.trace_no_menu.currentIndex(), graph_type="time"
            )
