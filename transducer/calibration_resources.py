...
def closest_frequency(frequency, filename, textbox: QTextBrowser):
    ...
    closest_frequency = str(
        find_nearest(frequencies, float(frequency))) + ending  # closest frequency to requested frequency
...
def eb50_dictionary(closest_frequency, filename):
    ...
    last_index = indices[
        next(x[0] for x in enumerate(indices) if x[1] >= first_index)]  # find the last line of relevant data
    eb50_data = lines[(
                              first_index + 2):last_index]  # this is the relevant data from the .txt for the
...
def fetch_data(filename, axial_or_lateral):
    ...
    with h5py.File(filename, "r") as f:
        ...
        if axial_or_lateral == 'axial':
            ...
            pressure = np.reshape(pressure, (y_data_hdf5.shape[0], z_data_hdf5.shape[0]))
        else:
            # reshape pressure array to fit x and z
            pressure = np.reshape(pressure, (x_data_hdf5.shape[0], z_data_hdf5.shape[0]))
...
def fwhmx(horizontal, pressure_or_intensity, left_field_length, right_field_length,
          axis, type_of_scan, type_of_data, textbox: QTextBrowser) -> Tuple[Union[float, str], float]:
    """
    FWHMX CALCULATION
    Return: Full width half max in mm (or error string), offset (or zero if error)
    """
    ...
    try:
        first_index = x[indices[np.argwhere(indices < max_location)[-1]]][
            0]  # first intersection with half_max_line, to the left of the peak
        last_index = x[indices[np.argwhere(indices > max_location)[0]]][
            0]  # last intersection with half_max_line, to the right of the peak
...
def create_sweep_file(sweep_list, save_folder, transducer, freq, save, eb50_file: str = "",
        ...
        with h5py.File(sweep_filename, "r") as f:
            ...
            try:
                ...
                if eb50_file == "":
                    ...
                    textbox.append(
                        "No power reading found in sweep file - switching to EB-50 file for power inference.\n")
    ...
    pressure_list_to_array = np.zeros(
        (len(pressures), len(pressures[0])))  # x dimensions: rows of pressures, y dimensions: columns of pressures
    ...
    for i in range(len(fn_gen_amplitudes[0])):
        fn_gen_amplitudes_list_to_array[:, i] = np.array(
            list((map(lambda x: x[i], fn_gen_amplitudes))))  # convert the tuple columns to numpy arrays
    ...
    # otherwise, take the power directly from the files
        ...
        powers_list_to_array = np.zeros(
            (len(powers), len(powers[0])))  # x dimensions: rows of pressures, y dimensions: columns of pressures
        # print(len(powers[0]))
        for i in range(len(powers[0])):
            powers_list_to_array[:, i] = np.array(
                list((map(lambda x: x[i], powers))))  # convert the tuple columns to numpy arrays
...
def field_graph(horizontal, vertical, pressure_or_intensity, left_field_length, right_field_length, field_height, name,
                type_of_scan, type_of_data, interp_step, save, save_folder, textbox: QTextBrowser):
    """
    FIELD PLOT
    """
    ...
    ax1.tick_params(axis="both", direction='in',
                    pad=7)  # sets the ticks to be inside the plot frame and also adds padding space between the axis and the tick labels
    ...
    interp = RegularGridInterpolator((horizontal, vertical), pressure_or_intensity, bounds_error=False, fill_value=None,
                                     method='cubic')  # interpolator method
    ...
    pcm = ax1.pcolormesh(X1, Y1, Z1, cmap=cmap, shading='nearest',
                         rasterized=True)  # the actual pcolormesh, rasterized NEEDS to be set to True to prevent SVGs saving weirdly
    ...
    interval = abs(
        (maximum - minimum) / 5)  # intervals of 1/5th to locate ticks on colorbar at 0, 0.2, 0.4, 0.6, 0.8, 1.
    positions = [minimum, minimum + interval, minimum + interval * 2, minimum + interval * 3, minimum + interval * 4,
                 maximum]  # where to put each tick on the colorbar
    ...
    if save:
        ...
        fig1.savefig(save_filename, bbox_inches='tight', format='svg', pad_inches=0,
                     transparent=True)  # pad_inches = 0 removes need to shrink image in Inkscape
...
def line_graph(horizontal, pressure_or_intensity, left_field_length, right_field_length, name, type_of_scan,
               type_of_data, save, save_folder, textbox: QTextBrowser):
    minimum = pressure_or_intensity.min()
    maximum = pressure_or_intensity.max()
    ...
    if left_field_length != "linear" and right_field_length != "linear":
        ax2.set_aspect(abs((-abs(left_field_length) - abs(right_field_length)) / (-2)) * (
                3.75 / 4))  # aspect ratio (FOR MANUAL ADJUSTMENT, change 3.75/4 to your desired ratio)
    ...
    norm_pressure = np.divide(pressure_or_intensity, maximum)  # normalisation of pressure to 1 and below
    maximum_location = np.where(norm_pressure == 1.0)  # where is the maximum pressure value?
    max_curve = maximum_location[1][
        0]  # which curve (column) does the maximum pressure value belong to in the norm_pressure array?
    ...
    if left_field_length != "linear" and right_field_length != "linear":
        ax2.plot(horizontal, new_pressure_or_intensity, color='#74BEA3', lw=2.8,
                 solid_capstyle='round')  # graph the maximum pressure curve
        # fig2.set_size_inches(7, 5)
    else:
        ax2.plot(horizontal, new_pressure_or_intensity, color='#74BEA3', lw=2.8, solid_capstyle='round',
                 marker='o')  # graph the maximum pressure curve
